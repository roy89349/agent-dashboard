      ...
      <DialogContent className="max-h-[85dvh] overflow-y-auto">
        ...
        <div className="mb-3 inline-flex rounded-lg border border-white/10 bg-black/20 p-0.5 text-sm">
          {(["existing", "new"] as const).map((m) => (
            <button key={m} onClick={() => setMode(m)} className={`rounded-md px-3 py-1 ${mode === m ? "bg-white/10 text-white" : "text-white/50"}`}>{m === "existing" ? "From registry" : "New / template"}</button>
          ))}
        </div>
        ...
        {mode === "existing" ? (
          <div className="max-h-72 space-y-1.5 overflow-y-auto">
            {existing.length === 0 ? <p className="py-6 text-center text-sm text-white/40">Every registered agent is already on this team.</p> : existing.map((a) => (
              <button key={a.id} onClick={() => { onAddMember(a.id); onClose(); }} className="flex w-full items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] p-2 text-left hover:border-white/25">
          ...
          <div className="space-y-3">
            <label className="block"><span className="mb-1 block text-xs text-white/45">Template</span>
              <select value={tplRole} onChange={(e) => pickTemplate(e.target.value)} className="h-9 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-white outline-none">
                {AGENT_TEMPLATES.map((t) => <option key={t.role} value={t.role} className="bg-[#0d1322]">{t.name}</option>)}
              </select>
            </label>
            <div className="grid grid-cols-2 gap-2">
              <label className="block"><span className="mb-1 block text-xs text-white/45">Name</span><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Frontend" className="h-9 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-white outline-none" /></label>
              <label className="block"><span className="mb-1 block text-xs text-white/45">Role</span><input value={role} onChange={(e) => setRole(e.target.value)} className="h-9 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-white outline-none" /></label>
            </div>
            <label className="block"><span className="mb-1 flex justify-between text-xs text-white/45">Agent id <span className="text-white/25">auto from name</span></span><input value={id} onChange={(e) => setId(e.target.value)} placeholder={slug(name || role)} className="h-9 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-white outline-none" /></label>
            ...
            <button onClick={create} disabled={busy} className="h-10 w-full rounded-xl bg-emerald-500 text-sm font-semibold text-black hover:bg-emerald-400 disabled:opacity-50">{busy ? "Creating…" : "Create + add to team"}</button>
          </div>
        )}
      </DialogContent>
    </Dialog>
